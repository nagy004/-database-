...
if [[ $pk_check == 0 ]]; then 
echo " " >> ~/database/1/$tname
echo -n "  $cv  : " >> ~/database/1/$tname
   no_of_col=($(head -n 1 ~/database/1/$tname | awk -F ":" '{print NF}'))
   for (( i = 2; i < no_of_col ; i++ )); do
		     read -p "Enter the( $i ) columne value : " Cname
                     if [[ $Cname =~ ^[0-9a-zA-Z]+$ ]]; then
                     
		     echo -n "$Cname : " >> ~/database/1/$tname
		     else
echo "invalid sentax "
break
fi
